...
<html>
	<head>
		...
		<!-- 
			...
			include 액션태그
			- 일반적으로 ui모듈을 삽입할 때 사용하는 태그
			- 동적타임에 삽입, include 지시자는 정적타임 
		-->
	</head>
	<body>
		<h3>include 액션태그</h3>
		
		<h4>include 지시자</h4> <!-- 정적타임. include 되어있음. 지시자를 더 많이 쓰는 것 같음 -->
		
		<%@ include file="/inc/_header.jsp" %>
		<%@ include file="/inc/_footer.jsp" %>
		<%@ include file="/inc/_config1.jsp" %> <!-- 삽입되어 있기 때문에 밑에 출력이 가능 -->
		...
		<h4>include 태그</h4> <!-- 더 깔끔함. 실행할 때 삽입이 됨. 동적타입 -->
		
		<jsp:include page="/inc/_header.jsp"/>
		<jsp:include page="/inc/_footer.jsp"/>
		<jsp:include page="/inc/_config2.jsp"/> <!-- 동적 타임에 삽입되어있어서 참조할 수 없음. -->
		
		<% // config2가 동적타임(실행상태)에 include 되기 때문에 정적타임에 참조가 불가능  
